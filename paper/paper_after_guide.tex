...
\begin{itemize}
	\item {Rule application action - indicates the application of a specific rule to the current goal, the action can be parameterized or not, depending on the rule.}
	\item {ok action - admits the current goal}
	\item {showGoal action - prints the current goal to the console - it does not modify the state of the prover}
\end{itemize}
...
\begin{itemize}
	\item {check} - marks the current goal to be checked against previously proven goals or axioms
	\item {implication} - marks the current goal to be run through the implication prover - in case the goal is an implication, try to prove it automatically
	\item {stop} - when there are no more sub-goals to be proven, the prover should stop and indicate the success of proving the root goal
\end{itemize}
