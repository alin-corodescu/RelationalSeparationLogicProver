...
\begin{itemize}
	...
	\item {\texttt{auto x}} action - instructs the prover to attempt to automatically prove an implication, search for a solution up to a depth of \texttt{x} rewrites
\end{itemize}
...
; 'begin '\n > .\end{lstlisting}
	\item For matching we also interpret the equality between variables using the \texttt{replace} operator introduced previously to reduce the \texttt{Goal} to a simpler form, which might match to an axiom.
