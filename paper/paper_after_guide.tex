...
\paragraph{}
	The semantics of the operators are standard, but a few clarifications are necessary to point out how our language constructs interact with the storage model, as well as a few differences from regular programming languages:
	\begin{itemize}
		\item{Verbosity of operators - most operators are replaced by their literal names (\(+\) becomes \(\mathtt{plus}\) etc.). I have opted for this approach to avoid syntactic complication when implementing the language in Maude, because most of the operators are already defined in Maude for its built-in types, and overloading them could cause conflicts since we are basing our own defined types on the primitives of Maude.}
...
\begin{appendices}
	...
	\label{proofExample}
	This appendix contains all the necessary commands to be run in a Maude console in order to run an example demonstration using the prover. The proven example is detailed in the \cite{relational} paper. 
\begin{lstlisting}
